...
-- Drop ALL existing SELECT policies on these tables to ensure clean slate
DO $$
DECLARE
    pol RECORD;
BEGIN
    -- Drop all SELECT policies for ai_generated_course_sections
    FOR pol IN
        SELECT policyname
        FROM pg_policies
        WHERE tablename = 'ai_generated_course_sections'
        AND cmd = 'SELECT'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON ai_generated_course_sections', pol.policyname);
    END LOOP;

    -- Drop all SELECT policies for ai_detected_topics
    FOR pol IN
        SELECT policyname
        FROM pg_policies
        WHERE tablename = 'ai_detected_topics'
        AND cmd = 'SELECT'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON ai_detected_topics', pol.policyname);
    END LOOP;
END $$;

-- Recreate SELECT policy for ai_generated_course_sections with shared exam support
...
-- Recreate SELECT policy for ai_detected_topics with shared exam support
